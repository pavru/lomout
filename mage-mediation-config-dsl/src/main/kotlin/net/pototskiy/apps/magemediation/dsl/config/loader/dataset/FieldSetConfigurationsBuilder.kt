...
class FieldSetConfigurationsBuilder {
    ...
    private fun validateAllFieldNameUnique() {
        ...
        if (dupNames.any { it.value.size > 1 }) {
            throw ConfigException(
                "Field names<${dupNames.filter { it.value.size > 1 }.keys.joinToString(
                    ", "
                )}> are duplicated in dataset"
            )
        }
    }
    ...
    private fun validateFieldSetHasUniqueName() {
        ...
        if (dupNames.any { it.value.size > 1 }) {
            throw ConfigException(
                "Field set names<${dupNames.filter { it.value.size > 1 }.keys.joinToString(
                    ", "
                )}> are duplicated"
            )
        }
