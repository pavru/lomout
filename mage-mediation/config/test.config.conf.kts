...
import net.pototskiy.apps.magemediation.plugins.medium.GroupPathFromRelationBuilder
...
config {
    ...
    loader {
        ...
        onecProduct(
            ...
            sources { source { file("onec_product").sheet("stock").stopOnEmptyRow() } }
            main("product") {
                field("sku") { column(0).pattern("^[0-9]+$") } to attribute { key() }
                field("description") { column(1) } to attribute { type { text() } }
                field("bool_val") { column(2) } to attribute { type { bool() } }
                field("int_val") { column(3) } to attribute { type { int() } }
                field("double_val") { column(4) } to attribute { type { double().locale("ru_RU") } }
                field("date_val") { column(5) } to attribute { type { date().pattern("d.M.yy") } }
                field("datetime_val") { column(6) } to attribute { type { datetime().pattern("d.M.yy H:m") } }
                field("string_list") { column(7) } to attribute { type { stringList().quote("").delimiter(",") } }
                field("bool_list") { column(8) } to attribute { type { boolList().quote("").delimiter(",") } }
                field("int_list") { column(9) } to attribute { type { intList().quote("").delimiter(",") } }
                field("double_list") { column(10) } to attribute {
                    type {
                        doubleList().locale("ru_RU").quote("").delimiter("|")
                    }
                }
                field("date_list") { column(11) } to attribute {
                    type {
        ...
        onec {
            group {
                ...
                pathAttribute {
                    ...
                    synthetic { klass(GroupPathFromRelationBuilder::class) }
                }
            }
        }
        magento {
            category {
                idAttribute("entity_id") { type { int() } }
                pathAttribute("path") {
                    type { string() }
                    root("")
                    separator("/", "/")
