...
import net.pototskiy.apps.lomout.api.script.loader.SourceSheetDefinition.SourceSheetDefinitionWithPattern
    ...
    class Builder(private val helper: ScriptBuildHelper) {
        ...
        fun file(id: String) {
            file = helper.definedSourceFiles.findRegistered(id)
                ?: throw AppConfigException(
                    suspectedLocation(),
                    message("message.error.script.load.source.file_not_defined", id)
                )
        }
        ...
        fun build(): SourceData = SourceData(
            this.file ?: throw AppConfigException(
                suspectedLocation(),
                message("message.error.script.load.source.file_not_defined", "")
            ),
            this.sheet,
