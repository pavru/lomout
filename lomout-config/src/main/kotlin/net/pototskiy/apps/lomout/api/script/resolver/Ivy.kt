...
package net.pototskiy.apps.lomout.api.script.resolver

import net.pototskiy.apps.lomout.api.MessageBundle.message
import net.pototskiy.apps.lomout.api.script.MainAndIdeLogger
...
class IvyResolver : GenericRepositoryWithBridge {
    ...
    fun tryResolve(
        ...
        with(artifactCoordinates) {
            ...
            logger.trace(message("message.trace.script.resolver.try_to_resolve", artifactId))
            val artifact = resolveArtifact(artifactId, excludes)
            if (artifact.isEmpty()) {
                logger.error(message("message.error.script.resolver.cannot_resolve", artifactId))
            } else {
                logger.trace(
                    message(
                        "message.trace.script.resolver.resolved",
                        artifactId,
    ...
    private fun GenericArtifactCoordinates.artifactId(): List<String> {
        return if (this is MavenArtifactCoordinates && (groupId.isValidParam() || artifactId.isValidParam())) {
            ...
            if (stringCoordinates.isValidParam() && stringCoordinates.count { it == ':' } == 2) {
                ...
                error(message("message.error.script.resolver.wrong_maven_args", stringCoordinates))
            }
        }
    }
