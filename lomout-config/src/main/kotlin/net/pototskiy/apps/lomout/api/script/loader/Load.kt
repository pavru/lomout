...
data class Load<T : Document>(
    ...
    class Builder<T : Document>(
         ...
         */
        fun sourceFields(block: FieldSetCollection.Builder<T>.() -> Unit) {
        ...
        fun build(): Load<T> {
            ...
            val sources =
                this.sources
                    ?: throw AppConfigException(
                        suspectedLocation(),
                        message("message.error.script.load.no_source", entityType.qualifiedName)
                    )
            ...
            return Load(
                ...
                fieldSets
                    ?: throw AppConfigException(
                        suspectedLocation(),
                        message("message.error.script.load.no_fieldset", entityType.qualifiedName)
                    )
            )
        }
        ...
        private fun validateFieldColumnDefinition() {
            ...
            if (this.headersRow == UNDEFINED_ROW && fields.any { it.key.column == UNDEFINED_COLUMN }) {
                throw AppConfigException(
                    suspectedLocation(),
                    message("message.error.script.field.column.not_defined",
