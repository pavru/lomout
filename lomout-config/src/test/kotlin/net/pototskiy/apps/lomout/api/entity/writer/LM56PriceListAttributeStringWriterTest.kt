...
internal class LM56PriceListAttributeStringWriterTest {
    internal class TestType : Document() {
        @Writer(Attr1Writer::class)
        var attr1: List<Price> = emptyList()
        @Writer(Attr2Writer::class)
        var attr2: List<Price> = emptyList()
        @Suppress("unused")
        var attr3: List<Price> = emptyList()

        companion object : DocumentMetadata(TestType::class)

        class Attr1Writer : WriterBuilder {
            override fun build(): AttributeWriter<out Any?> = createWriter<PriceListAttributeStringWriter> {
                quotes = null
                delimiter = ','
            }
        }
    ...
    internal fun simpleWriteUnquotedTest() {
        ...
        val value = listOf(Price(11.22), Price(33.44))
        assertThat(cell.cellType).isEqualTo(CellType.BLANK)
        @Suppress("UNCHECKED_CAST")
        (attr.writer as AttributeWriter<List<Price>>).write(value, cell)
    ...
    internal fun writeNullValueTest() {
        ...
        assertThat(cell.cellType).isEqualTo(CellType.BLANK)
        @Suppress("UNCHECKED_CAST")
        (attr.writer as AttributeWriter<List<Price>?>).write(null, cell)
        assertThat(cell.cellType).isEqualTo(CellType.BLANK)
    }

    @Test
    internal fun defaultWriterTest() {
        val writer = defaultWriters[SupportAttributeType.priceListType]
        assertThat(writer).isNotNull
        assertThat(writer).isInstanceOf(PriceListAttributeStringWriter::class.java)
        writer as PriceListAttributeStringWriter
