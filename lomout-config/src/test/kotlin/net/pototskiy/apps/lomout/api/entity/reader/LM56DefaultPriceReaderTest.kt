...
internal class LM56DefaultPriceReaderTest {
    ...
    internal fun readBlankCellTest() {
        val reader = PriceAttributeReader().apply { locale = "en_US" }
    ...
    internal fun readDoubleCellTest() {
        ...
        assertThat(reader.read(attr, inputCell)).isEqualTo(Price(1.1))
    }
    ...
    internal fun readLongCellTest() {
        ...
        assertThat(reader.read(attr, cell)).isEqualTo(Price(11.0))
    }
    ...
    internal fun readBooleanCellTest() {
        ...
        assertThat(reader.read(attr, inputCell)).isEqualTo(Price(1.0))
        xlsTestDataCell.setCellValue(false)
        assertThat(inputCell.cellType).isEqualTo(CellType.BOOL)
        assertThat(reader.read(attr, inputCell)).isEqualTo(Price(0.0))
    }

    @Test
    internal fun readStringEnUsCellTest() {
        val readerEnUs = PriceAttributeReader().apply { locale = "en_US" }
        val readerRuRu = PriceAttributeReader().apply { locale = "ru_RU" }
        xlsTestDataCell.setCellValue("1.1")
        assertThat(inputCell.cellType).isEqualTo(CellType.STRING)
        assertThat(readerEnUs.read(attr, inputCell)).isEqualTo(Price(1.1))
    ...
    internal fun readWithWorkbookLocaleTest() {
        ...
        val reader = PriceAttributeReader().apply {
            locale = null
        }
        assertThat(reader.read(attr, inputCell)).isEqualTo(Price(3.3))
    }

    @Test
    internal fun defaultPriceReaderTest() {
        @Suppress("UNCHECKED_CAST")
        val reader = defaultReaders[SupportAttributeType.priceType]
        assertThat(reader).isNotNull
        assertThat(reader).isInstanceOf(AttributeReader::class.java)
        reader as PriceAttributeReader
